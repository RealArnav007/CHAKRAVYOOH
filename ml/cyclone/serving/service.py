...
def create_app() -> Any:
    ...
    if not FASTAPI_AVAILABLE:
        raise RuntimeError(
            "FastAPI is not installed. Please install fastapi and uvicorn to use the HTTP server."
        )
    ...
    def get_next_frame(storm_id: str) -> dict[str, Any]:
        ...
        if not frames:
            raise HTTPException(
                status_code=404, detail=f"No replay frames found for storm '{storm_id}'."
            )
    ...
    def reset_replay(
        storm_id: str,
        jump: str | None = Query(
            None, description="Optional preset jump (e.g. 'landfall-24h', 'genesis')"
        ),
    ) -> dict[str, Any]:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Serve Chakravyuh Cyclone Intelligence Replay HTTP API."
    )
    parser.add_argument(
        "--host", type=str, default="0.0.0.0", help="Host bind address (default: 0.0.0.0)."
    )
    parser.add_argument("--port", type=int, default=8000, help="Port to listen on (default: 8000).")
    parser.add_argument(
        "--storm", type=str, default="Amphan", help="Default storm to precompute on startup."
    )
    args = parser.parse_args()

    if not FASTAPI_AVAILABLE:
        print(
            "[ERROR] FastAPI and Uvicorn are required to start the HTTP server. Run: pip install fastapi uvicorn"
        )
        return
