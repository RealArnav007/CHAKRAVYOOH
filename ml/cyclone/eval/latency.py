...
def profile_engine_latencies(
    ...
    from ml.cyclone.replay.replay import replay
    ...
    env_t = (
        ...
        .unsqueeze(0)
        .to(dtype=torch.float32, device=target_device)
    )
    track_t = (
    ...
    for _ in range(num_iterations):
        ...
        with torch.no_grad():
            out = model(
                img=img_t, env_vector=env_t, track_sequence=track_t, image_available=avail_t
            )
            # Calibration scaling
            calibrator.detection_scaler.predict_proba(
                out["detection_logits"].cpu().numpy(), is_binary=True
            )
            calibrator.stage_scaler.predict_proba(
                out["stage_logits"].cpu().numpy(), is_binary=False
            )
            calibrator.intensity_scaler.predict_proba(
                out["intensity"]["imd_logits"].cpu().numpy(), is_binary=False
            )
        if target_device == "cuda":
            torch.cuda.synchronize()
        t1 = time.perf_counter()
...
def main() -> None:
    ...
    parser.add_argument(
        "--iterations", "-n", type=int, default=50, help="Number of benchmark iterations."
    )
    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="ml/cyclone/artifacts/latency_profile.json",
    ...
    print("=" * 65)
    print(
        f"  Tier-0 Deterministic Baselines : Mean: {profile['tier0_baseline']['mean_ms']} ms | P95: {profile['tier0_baseline']['p95_ms']} ms"
    )
    print(
        f"  Tier-1 Neural FusionNet + Cal  : Mean: {profile['tier1_fusion_net']['mean_ms']} ms | P95: {profile['tier1_fusion_net']['p95_ms']} ms"
    )
    print(
        f"  Full run_cycle() Orchestrator  : Mean: {profile['full_run_cycle_pipeline']['mean_ms']} ms | P95: {profile['full_run_cycle_pipeline']['p95_ms']} ms"
    )
